...
The problem addressed by this thesis falls within that regime, but only because the two decisions it couples are not both searched exactly: the combinatorial explosion belongs to the discrete choice of which template each article receives, which the evolutionary search of explores directly, not to the placement problem left once that choice is fixed. Once every article has an assigned template, what remains is a standard two-dimensional packing instance over the small number of articles a single page holds (tipically less than 10 according to \melody), which is inside the regime in which exact placement stays tractable.

It is on this basis, backtracking exhaustively and completely over extreme-point placements (\secref{sec:sota-packing-heuristics}), that Chapter~\ref{chapter:pagegen} resolves placement exactly rather than through the constructive heuristics reviewed above.
