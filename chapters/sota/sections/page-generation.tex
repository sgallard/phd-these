...
NSGA-III~\cite{nsga-iii-pt1} addresses this by replacing relative density with an absolute, geometric notion of diversity, while keeping everything else from NSGA-II unchanged: the same fast non-dominated sorting, and the same elitist replacement that fills $P_{t+1}$ front by front. Only the last, partially admitted front is now selected differently: instead of favoring the least crowded solutions via $\prec_n$, NSGA-III associates every candidate with a predefined set of reference directions spread over the objective space, and fills the remaining slots by preserving under-represented directions rather than under-represented neighborhoods.
...
The problem addressed by this thesis, packing a heterogeneous set of rectangular articles onto a page, is a two-dimensional, rectangular packing problem, consisting of two basic variants which are examined following the terminology established by Lodi et al.~\cite{lodi-packing-survey}:
...
The problem addressed by this thesis falls within that regime, but only because the two decisions it couples are not both searched exactly: the combinatorial explosion belongs to the discrete choice of which template each article receives, which the evolutionary search of Chapter~\ref{chapter:pagegen} explores directly, not to the placement problem left once that choice is fixed. Once every article has an assigned template, what remains is a standard two-dimensional packing instance over the small number of articles a single page holds (typically fewer than 10 according to \melody), which is inside the regime in which exact placement stays tractable. It is on this basis, backtracking exhaustively and completely over extreme-point placements (\secref{sec:sota-packing-heuristics}), that Chapter~\ref{chapter:pagegen} resolves placement exactly rather than through the constructive heuristics reviewed above.
...
\end{itemize}
In both cases,  the surrogate exists to make an expensive evaluation cheaper.
