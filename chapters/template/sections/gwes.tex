...
In the global page-aware context, template evaluation shifts from isolated single-article selection to assessing candidate template assignments across all articles assigned to a unified page canvas $\page$ (\secref{sec:sota-page-aware-scoring}). This requirement stems from the fact that professional newspaper layouts exhibit tacit structural and visual dependencies, where selecting a template $\templatesel{i}$ for a given article $\articleobj{i}$  depends upon, and influences, the neighboring articles on the same page.

Evaluating template suitability at the page level requires simultaneously addressing two complementary factors: modeling the internal topological structure of individual templates and capturing the contextual interdependencies among the set of articles $\articleset = \{\articleobj{1}, \articleobj{2}, \dots, \articleobj{n}\}$ co-existing on the page. Standard flat metadata metrics and single-article mechanisms are insufficient for this task because they evaluate candidate templates in isolation, failing to capture how spatial configurations interact across adjacent layout blocks to form a cohesive visual spread.

%%%%%% From here keep it
Rather than relying on rigid heuristic rules, this challenge is addressed by developing a neural evaluation metric that scores the quality of assigned templates $\templatesel{i}$ for each article $\articleobj{i}$ in the page $\page$, considering also their placement. This module dynamically scores the contextual harmony between an article's content and its spatial configuration within a newspaper layout, combining a Transformer encoder with a precalculated template similarity matrix (using $\gmn{\cdot}{\cdot}$) into a single neural method coined \NeuralMethod\ (\neuralmethod) (\secref{sec:sota-surrogate}). Because this score must be evaluated repeatedly by the page generation method of Chapter~\ref{chapter:pagegen}, it is formulated as a page-level scoring function rather than as a retrieval mechanism, like \trs.
...
\begin{enumerate}
    \item \textbf{Input Metadata:} The intrinsic structural features of article $\articleobj{i} \in \articleset$ exactly as defined in Chapter~\ref{chapter:definitions}. This vector encapsulates four concrete numerical features extracted from the input instance: the body text character count, the total number of allocated images, and binary flags indicating the presence of an article title and subtitle, respectively.
    
    \item \textbf{Spatial Coordinates:} The absolute geometric coordinates of each article $\articleobj{i} \in \articleset$, relative to the page dimensions (\Wpage\ and \Hpage). This vector captures the physical bounding box properties of $\articleobj{i}$, specifically its top-left grid coordinates $(x_i, y_i)$, width $\Wtemp{\templatesel{i}}$, height $\Htemp{\templatesel{i}}$, and the number of page columns spanned by the assigned template.
...
In this section the predictive capabilities and structural behavior of the components underlying the \neuralmethod\ metric are verified. Beyond raw predictive accuracy, this evaluation specifically targets the shape of the resulting probability landscape over template alternatives, since it is this landscape, not a single top prediction, that the evolutionary operators of Chapter~\ref{chapter:pagegen} explore across generations.
...
Following the filtering and classification pipeline described in \secref{sec:dataset-preprocessing}, $4,449$ usable pages from the \laprovence archive are available to train the Transformer Encoder module. These instances contain a variable count of between $1$ and $11$ distinct news articles arranged simultaneously within a single page canvas; the feature streams extracted from each article $\articleobj{i}$ to build the network's input are detailed in the input tensor paragraph of \secref{sec:transformer_module}.

\paragraph{Template Catalog}

The structural template catalog $\templatecatalog$ used to compute $\gmnMatrix$ is the \laprovence catalog described in \secref{sec:laprovence}, comprising $\catalogsize = 345$ templates.
...
This reuse is justified by construction: since $\gmn{\cdot}{\cdot}$ is trained to capture the topology of zoning blocks and their semantic classes $\blockfeat{\cdot}$ rather than catalog-specific details, it transfers directly to any catalog sharing this same block taxonomy and zoning representation, such as \laprovence relative to \publihebdos, without requiring catalog-specific retraining.
...
Although the standalone Transformer Encoder achieves high predictive accuracy, its raw probability distributions are often highly concentrated. To evaluate how the  integration of the LayoutGMN affinity matrix~$\gmnMatrix$ relaxes this constraint, an exhaustive analysis was executed over the test set. 

This validation measures the \emph{smoothing} effect of the unified \neuralmethod\ metric by capturing flexibility gains across the dataset. For mathematical rigor, because the raw matrix multiplication $\gmnMatrix \cdot \mathbf{V}_{T}^{(i)}$ yields non-normalized structural affinity values, the score vectors were temporarily normalized for the statistical computation to ensure an equivalent probability density domain. This calibration is strictly analytical and restricted to the calculation of these metrics, allowing a fair baseline comparison. The quantitative results of this analysis are summarized in Table~\ref{tab:gmn_impact}.
...
This behavioral characteristic is qualitatively illustrated in \figref{fig:gwes-vs-transformer}. As shown in this representative example, the human-selected target template (\texttt{T:594763}) is successfully identified as the top rank by both paradigms. However, the standalone Transformer exhibits a rigid, steep probability drop-off immediately after its first choice, collapsing from $0.478$ down to $0.106$ and $0.035$ for the subsequent positions. This polarization penalizes structurally similar alternatives, severely limiting the exploration capacity if the top candidate has conflicts.
Conversely, the unified \neuralmethod\ metric leverages the graph-matching prior to build a cooperative, high-utility plateau; the target template achieves a dominant affinity score of $0.619$, while its direct visual twin (\texttt{T:594765}) is elevated to an equivalent affinity score of $0.605$.

\subsection{Discussion}
\label{sec:gwes-discussion}

\subsubsection*{Historical Data Dependency}
...
In this thesis, however, this bottleneck did not materialize: the \laprovence archive provided a sample large enough to train the Transformer encoder to the accuracy levels reported in \secref{sec:gwes-evaluation}. Whether this data requirement generalizes favorably to smaller or less digitized archives remains an open question for future deployments.
