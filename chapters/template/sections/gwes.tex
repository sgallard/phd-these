...
\subsection{$\gmn{\cdot}{\cdot}$ inclusion for template similarity}
...
To mitigate this, \neuralmethod\ incorporates a precalculated pairwise similarity matrix $\gmnMatrix = [s_{ij}] \in \mathbb{R}^{\catalogsize \times \catalogsize}$ derived from $\gmn{\cdot}{\cdot}$ and $\gmnNetwork$~\cite{patil-2021, li-gmn-2019}.
...
\end{equation}
where $\sigmagmn$ represents a tunable scaling parameter calibrated according to the dataset characteristics to control the smoothness of the topological mapping. This transformation yields the final similarity matrix $\gmnMatrix$, with a bounded similarity score $s_{ij} \in [0, 1]$ between any pair of templates.

\subsection{\neuralmethod\ calculation}
...
In this section the predictive capabilities and structural behavior of the deep learning components underlying the \neuralmethod\ metric are verified. 
...
\subsubsection{Page Layout and Template Catalog Dataset (\laprovence)}

To evaluate the architectural robustness and industrial applicability of the proposed framework, empirical validations were conducted using real-world archives from La Provence~\cite{laprovence}, a prominent French daily newspaper group. This comprehensive production repository was processed and decoupled into two primary interconnected structural components: a historical layout dataset and a static template catalog~$\templatecatalog$. The historical layout dataset  provides the required historical context to train the Transformer Encoder module of \neuralmethod. Concurrently, the static template catalog~$\templatecatalog$ serves as the underlying structural dataset utilized to compute the $\gmnMatrix$ matrix.
...
The structural template catalog $\templatecatalog$ is composed by  $\catalogsize = 345$ designs. Each blueprint specifies all elements described in Chapter~\ref{chapter:definitions} for a given template: dimensions ($W_j, H_j$), internal zoning $\zoning{j}$ and text capacity mapping $\capacity{j}$. 


\subsubsection{$\gmnMatrix$ calculation}

The structural similarity matrix $\gmnMatrix$ is computed utilizing the template catalog $\templatecatalog$ from \laprovence, but using an instance of $\gmnNetwork$ pre-trained on the analogous, structurally equivalent publishing corpus of \publihebdos, detailed in Chapter~\ref{section:trs-eval}. 
...
The operational parameters governing the neural training of the Transformer Encoder in \neuralmethod\  were calibrated through extensive empirical pilot studies over the specified dataset. 
...
Early stopping epoch threshold & --- & 40  \\ \hline
\end{tabularx}

\caption{Calibrated algorithmic and structural hyperparameters for the Transformer Encoder training.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\widthForOneImage]{img/gwes_vs_transformer_ex2.png}
...
This behavioral characteristic is qualitatively illustrated in Figure~\ref{fig:gwes-vs-transformer}. As shown in this representative example, the human-selected target template (\texttt{T:594763}) is successfully identified as the top rank by both paradigms. However, the standalone Transformer exhibits a rigid, steep probability drop-off immediately after its first choice, collapsing from $0.478$ down to $0.106$ and $0.035$ for the subsequent positions. This extreme polarization penalizes structurally similar alternatives, severely limiting the exploration capacity of the evolutionary engine if the top candidate encounters conflicts.


Conversely, the unified \neuralmethod\ metric leverages the graph-matching prior to build a cooperative, high-utility plateau. To reflect the true operational reality of the evolutionary process, the values displayed for \neuralmethod\ in Figure~\ref{fig:gwes-vs-transformer} correspond to the raw, non-normalized affinity scores rather than formal probabilities. Under this framework, \neuralmethod\ actively expands the search space by shifting values; the target template achieves a dominant affinity score of $0.619$, while its direct visual twin (\texttt{T:594765}) is elevated to an equivalent utility level of $0.605$, and alternative valid blueprints like \texttt{T:594748} are uncovered at $0.541$.
