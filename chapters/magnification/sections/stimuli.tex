...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\widthForOneImage]{img/newspaper-material.pdf}
    \caption{
    ...
    Colors indicate matching articles across the two versions (i.e., occupying the same area on the page). The textual content differs between (a) and (b) to avoid repetition effects, but was balanced in headline length. The left-hand side illustrates how the original edition would look on a phone, which is the display used to define the headline size in LogMAR.}
...
For each original edition, a large-print counterpart was generated by applying the automated re-layouting method developed by Gallardo et al.~\citemine{gallardo}. This method predates the multi-objective framework later developed in Chapter~\ref{chapter:pagegen}, and both instantiate the same underlying principle: 
\begin{itemize}
    \item An evolutionary search that positions articles, each with an assigned \emph{shape}, onto the page canvas, preserving a two-dimensional layout format.
    \item Each article \emph{shape} was drawn from a discrete catalog of shapes.
\end{itemize}

In this earlier method, that catalog of shapes takes the form of a few dozen aspect-ratio variations of a rectangular block, rather than the hundreds of templates $\templatecatalog$ considered in this thesis. On the other hand, the search optimizes a single, magnification-specific objective that combines minimizing the number of lines in the headline with aesthetic quality (edge alignment and visual balance), rather than the four objectives later formalized in \secref{sec:objective_formulation}.

In particular, the method receives as input an original edition along with a target magnification factor, deploying an evolutionary algorithm to reorganize article blocks on the canvas. This optimization process simultaneously minimizes headline line wrapping and maximizes aesthetic quality metrics (e.g., edge alignments and visual balance). Crucially, the primary objective of this adaptation is structural integrity rather than the preservation of absolute spatial coordinates; while article containers are repositioned and resized to accommodate magnified typography, the underlying 2D layout topology and journalistic hierarchy are maintained.

For the purpose of generating large-print editions, the algorithm was provided with each baseline layout $l_i$ and a representative content set $c_{i,1}$, alongside a magnification factor of $\times 2$, which comfortably covers the critical print size spectrum across both participant cohorts (see \secref{sec:condition-definition}). Only $c_{i,1}$ was required for layout generation, as content sets $c_{i,2}$ and $c_{i,3}$ are strictly equivalent in character length. The output of this procedure is a set of optimized large-print layouts $\largeprintset = \{l'_i\}_{i=1,\dots,9}$ (\figref{fig:newspaper-pages}(b)). Each final large-print edition was obtained by pairing an optimized layout with one of the three equivalent content files for that page. It should be noted that in the large-print edition, an enlarged font size inherently limits the amount of body text displayed on the overview spread. This represents an intrinsic operational characteristic of digital large-print editions---analogous to commercial kiosk news applications---which are engineered primarily to facilitate page scanning and article discovery. Readers utilize the multi-article overview to locate entry points (i.e., headlines) and subsequently transition to a dedicated single-article reading mode for full content consumption.


\subsection{CPS-based reading conditions}
...
The visual angle of headlines depends on two factors: the device's screen size and the viewing distance. Given a participant's CPS, the goal is therefore to select the appropriate device and distance so that the headline size of the original edition in \condonenot is smaller than the CPS, while the headline size of the large-print edition in \condtwonot is larger than the CPS. When multiple configurations are possible, the one that yields headline sizes closest to the CPS is systematically chosen. This principle is illustrated in \figref{fig:conditions-setup}(a).
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\widthForOneImage]{img/conditions-setup.pdf}
    \caption{
    ...
    (b) Device and viewing distance configuration depending on the scenario: for participants \NV ($0 \leq CPS \leq 0.3$), the experiment was conducted on a phone at a fixed distance of 40\,cm; for participants \LV ($0.3 < CPS \leq 0.9$), the experiment was conducted on a tablet, with the viewing distance adjusted according to the CPS value.}
    \label{fig:conditions-setup}
\end{figure}

A custom-designed app, developed using the React Native framework, was deployed for the application of this protocol.
Regarding this, two different Android devices are used:
\begin{itemize}
