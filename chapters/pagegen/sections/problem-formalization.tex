...
Given this input, the desired output is a complete layout solution \solution, aggregating for every article $\articleobj{i} \in \articleset$ a template selection $\templatesel{i}$, a position allocation $\posvar{i}$, and a vertical elasticity $\elasticity{i}$ (formally defined in \secref{sec:decision-variables}) subject to the defined feasibility constraints and optimizing along the multi-objective vector $\objvector{\solution}$.
...
In particular, $\Omega(\articleobj{i}, \templateobj{\templatesel{i}} \mid \solution)$ is modeled using the \NeuralMethod (\neuralmethod) defined in \secref{sec:gwes}. Thanks to the page-aware context deep learning architecture, \neuralmethod\ is capable of capturing the complex interdependencies between neighboring articles and their assigned templates, effectively learning the implicit editorial style, thus resolving \objtwo. Of course, this assumes that the model has been trained on a sufficiently large and representative historical dataset of newspaper layouts, in the exact same way as described in \secref{sec:gwes}. Given this, $\Omega(\articleobj{i}, \templateobj{\templatesel{i}} \mid \solution)$ is formally defined as:
