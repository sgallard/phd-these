...
\subsubsection{Aspiration points dataset}
...
\subsubsection{Instance selection setup}
...
The asymmetric population sizes reported in Table~\ref{tab:algorithmic_parameters} ($\popsize = 286$ versus $\popsize_R = 330$) are not arbitrary hyperparameter selections; rather, they are strictly dictated by the underlying combinatorial requirements of the reference direction generation methods:
\begin{enumerate}
    \item For \methodNameUniform, deploying the canonical Das-Dennis boundary intersection method with $10$ structured partitions across the $4$-dimensional objective space yields exactly $\binom{4 + 10 - 1}{10} = 286$ unique reference directions, necessitating an identical population size to guarantee niche coverage. 
    \item Conversely, \methodName\ projects a structured mesh of $8$ internal partitions around each of the $2$ aspiration points, which generates exactly $165$ localized reference directions per target zone, culminating in a combined population tracking space of $165 \times 2 = 330$ individuals to preserve directional diversity. The deliberate selection of $8$ internal partitions for the preference-guided approach was specifically enforced to maintain an equitable computational budget and search capacity per generation, ensuring a balanced comparative analysis between both methods.
\end{enumerate}
...
\midrule
\multirow[c]{6}{*}{2} & B1 & \cellcolor{red!15}1.62 $\pm$ 0.49 & 0.81 $\pm$ 0.31 & 1.89 $\pm$ 1.67 & 0.38 $\pm$ 0.25 & \cellcolor{green!15}1.71 $\pm$ 0.37 \\
...
\multirow[c]{6}{*}{3} & B1 & \cellcolor{red!15}2.28 $\pm$ 0.73 & 1.11 $\pm$ 0.48 & 2.37 $\pm$ 1.38 & 0.47 $\pm$ 0.30 & 2.55 $\pm$ 0.41 \\
 ...
 & B3 & \cellcolor{red!15}1.14 $\pm$ 0.35 & \cellcolor{green!15}0.36 $\pm$ 0.36 & \cellcolor{green!15}\bfseries0.00 $\pm$ 0.00 & 0.66 $\pm$ 0.18 & \cellcolor{green!15}2.50 $\pm$ 0.28 \\
...
\multirow[c]{6}{*}{4} & B1 & \cellcolor{red!15}2.87 $\pm$ 0.82 & 1.39 $\pm$ 0.49 & 2.58 $\pm$ 0.96 & 0.54 $\pm$ 0.33 & 3.43 $\pm$ 0.47 \\
 & B2 & \cellcolor{red!15}1.40 $\pm$ 0.49 & 0.65 $\pm$ 0.45 & \cellcolor{green!15}\bfseries0.01 $\pm$ 0.03 & 0.65 $\pm$ 0.16 & 3.12 $\pm$ 0.38 \\
 & B3 & \cellcolor{red!15}1.40 $\pm$ 0.49 & \cellcolor{green!15}0.65 $\pm$ 0.45 & \cellcolor{green!15}\bfseries0.01 $\pm$ 0.03 & 0.65 $\pm$ 0.16 & \cellcolor{green!15}2.85 $\pm$ 0.35 \\
 ...
 & \methodNameUniform & \bfseries1.00 $\pm$ 0.00 & \cellcolor{green!15}0.08 $\pm$ 0.03 & 0.44 $\pm$ 0.27 & \cellcolor{green!15}\bfseries0.23 $\pm$ 0.15 & \cellcolor{green!15}\bfseries1.24 $\pm$ 0.32 \\
  \hline
...
\multirow[c]{6}{*}{8} & B1 & \cellcolor{red!15}4.63 $\pm$ 1.38 & 2.07 $\pm$ 0.79 & 3.25 $\pm$ 0.73 & 0.57 $\pm$ 0.30 & 7.02 $\pm$ 0.63 \\
 ...
 & B3 & \cellcolor{red!15}2.00 $\pm$ 0.00 & 1.15 $\pm$ 0.00 & \cellcolor{green!15}\bfseries0.00 $\pm$ 0.00 & 0.60 $\pm$ 0.00 & \cellcolor{green!15}5.87 $\pm$ 0.00 \\
...
To isolate and quantify the exact architectural contribution of the deep learning component within the optimization engine, an ablation analysis is conducted between the ablated method ($\baseline{5}$) and the proposed \methodNameUniform. Since both methods operate under the multi-objective paradigm and generate full Pareto frontiers rather than single solutions, traditional scalar metrics are insufficient. Instead, their performance is evaluated directly in their multi-objective domain using the Hypervolume (HV) indicator~\cite{hv}.

Intuitively, HV measures how much of the objective space a frontier covers toward improvement, and is calculated as follows (\figref{fig:hv-illustration}):
\begin{itemize}
  \item A reference point $z^{\text{ref}}$ is fixed once, slightly worse than every objective's worst attainable value.
  \item Then, every solution in a frontier $A$ dominates the box of objective space delimited by itself and $z^{\text{ref}}$, and $HV(A)$ is the volume of the union of all these boxes across the frontier: $$HV(A) = \Lambda\big(\bigcup_{a \in A} [a, z^{\text{ref}}]\big).$$
\end{itemize}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        % Hypervolume: union of boxes delimited by each point in A and z^{\text{ref}}
        \fill[blue!12] (1,5.8) -- (7.5,5.8) -- (7.5,1) -- (6.5,1) -- (6.5,1.8) -- (5,1.8) -- (5,2.5) -- (3.5,2.5) -- (3.5,3.5) -- (2,3.5) -- (2,5) -- (1,5) -- (1,5.8) -- cycle;
        \node[blue!40!black] at (5.3,4.6) {\footnotesize $HV(A)$};
        ...
        % Pareto front A: staircase through the non-dominated points
        \draw[thick] (1,5) -- (2,5) -- (2,3.5) -- (3.5,3.5) -- (3.5,2.5) -- (5,2.5) -- (5,1.8) -- (6.5,1.8) -- (6.5,1);

        % Non-dominated points forming A
        \node[paretopt] at (1,5) {};
        \node[paretopt] at (2,3.5) {};
        \node[paretopt] at (3.5,2.5) {};
        \node[paretopt] at (5,1.8) {};
        \node[paretopt] at (6.5,1) {};

        % Dominated points, excluded from A
        ...
        % Reference point, projected onto the axes
        \node[refpt, label={above right:$z^{\text{ref}}$}] at (7.5,5.8) {};
        \draw[dotted] (7.5,5.8) -- (7.5,0);
        \draw[dotted] (7.5,5.8) -- (0,5.8);
    \end{tikzpicture}
    \caption{Hypervolume (HV) of a non-dominated frontier $A$ (black points) with respect to a reference point $z^{\text{ref}}$, in an illustrative bi-objective minimization problem. Each point $a \in A$ delimits a box $[a, z^{\text{ref}}]$; the shaded region is the union of these boxes, and $HV(A) = \Lambda\big(\bigcup_{a \in A} [a, z^{\text{ref}}]\big)$. Gray points are dominated and therefore excluded from $A$.}
    \label{fig:hv-illustration}
\end{figure}

\paragraph*{Convergence}

Before analyizing the HV quality, it is necessary to evaluate the overall convergence of both evolutionary approaches. Since $\baseline{5}$ never optimizes for $\objtwo$, this convergence analysis is restricted to the reduced 3D objective subspace $(\objone, \objthree, \objfour)$, so that the resulting curves remain comparable between both methods. \figref{fig:convergence_analysis} tracks this 3D hypervolume evolution over $\ngen = 300$ generations for two representative instances, $\numarticles = 4$ and $\numarticles = 8$.

\begin{figure}[htbp]
\centering
\begin{subfigure}[b]{\widthForOneImage}
    \centering
    \includegraphics[width=\widthForOneImage]{img/results/convergence_curve_4.png}
    \caption{$\numarticles = 4$.}
    \label{fig:convergence_analysis_4}
\end{subfigure}
\vskip 0.8em
\begin{subfigure}[b]{\widthForOneImage}
    \centering
    \includegraphics[width=\widthForOneImage]{img/results/convergence_curve_8.png}
    \caption{$\numarticles = 8$.}
    \label{fig:convergence_analysis_8}
\end{subfigure}
\caption{Generational convergence analysis tracking the 3D hypervolume ($\objone, \objthree, \objfour$) over 300 generations, for a moderate ($\numarticles=4$) and a high complexity ($\numarticles=8$) instance. Solid lines represent the mean trajectory across all independent runs, while the shaded regions denote the standard deviation.}
\label{fig:convergence_analysis}
\end{figure}

As illustrated in \figref{fig:convergence_analysis}, both methods reach a stable optimization plateau well before the $\ngen$ threshold, with the $\numarticles=4$ instance (\figref{fig:convergence_analysis_4}) converging earlier than the $\numarticles=8$ instance (\figref{fig:convergence_analysis_8}).
...
Conversely, \methodName\ successfully counteracts this. By anchoring the survival selection around preference-derived aspiration points, \methodName\ concentrates its population within the target zone, maintaining remarkably high densities, with the gap over \methodNameUniform\ ranging from $+17.1$ percentage points at $\numarticles=3$ ($86.0\%$ vs. $68.9\%$) up to $+51.4$ percentage points at $\numarticles=6$ ($58.1\%$ vs. $6.7\%$). When cross-referenced with the cardinality reduction shown in Table~\ref{tab:frontier_cardinality_comparison}, these results deliver a definitive conclusion: \methodName\ does not suffer from a loss of valuable diversity, but rather performs a highly targeted pruning of non-viable space, ensuring the generated alternatives are strictly tailored to the newsroom's operational requirements. To confirm this behavioral shift, a Wilcoxon signed-rank test across all instances was conducted. The test decisively rejects the null hypothesis ($W = 5.0$, $p < 0.001$, effect size of $r = 1.03$), validating that the AE density superiority of \methodName\ is statistically significant.
...
\centering
    ...
    \end{subfigure}
\caption{Visual trade-off comparison using parallel coordinates for a representative medium-complexity instance ($\numarticles=5$) under shared global axis scaling. The gray shaded region represents the AE, while the black lines denote the aspiration points $r^{(k)}$ targeted by \methodName\ (\secref{sec:sota-rnsga3}); the boxplot overlaid on each axis is included for reference, showing the historical distribution from which the corresponding aspiration point was derived. The color gradient reflects the performance on $\objone$. (a) shows \methodNameUniform's non-dominated solutions scattered widely across the objective space, largely outside the AE; (b) shows \methodName's solutions concentrated within the AE, closely following the aspiration points.}
...
\subsection{Comparison with original input}
\label{sec:comparison-with-input}

Together with the qualitative results presented next, this subsection is the only place in this chapter where the complete input page is used, rather than only the content and constraints it was stripped down to. As already noted when the instances were introduced above, \methodName\ never observes this original layout or its true objective scores during optimization: it only receives the formal input vector $\instance = \langle \page, \articleset \rangle$. 
Dominating the input page was consequently never the design objective of the system; Table~\ref{tab:input_dominance} nonetheless measures how often the generated frontier happens to Pareto-dominate it.

\begin{table}[htbp]
\centering
\begin{tabular}{lcc}
\toprule
$\numarticles$ & Full 4D ($\objvector$) & 3D, excluding $\objone$  \\
\midrule
2 & 0.00\% & 99.52\% \\
3 & 4.29\% & 95.71\% \\
4 & 4.67\% & 58.67\% \\
...
\bottomrule
\end{tabular}
\caption{Pareto dominance of \methodName\ over the original input page. Percentage of optimization runs (seeds) in which \methodName's frontier contains at least one solution that Pareto-dominates the input, stratified by article count ($\numarticles$). The left column evaluates the full 4D objective space $\objvector$; the right column excludes $\objone$ to isolate its effect, restricting the comparison to the reduced 3D subspace $(\objtwo,\objthree,\objfour)$.}
\label{tab:input_dominance}
\end{table}

As shown in the left column of Table~\ref{tab:input_dominance}, \methodName\ almost never Pareto-dominates the input page when the full objective vector $\objvector$ is considered, with dominance rates below $5\%$ across every stratum. This is explained by $\objone$: since the human designer composes the page with full visibility of its final geometry, the resulting layout wastes essentially no space, a level of geometric efficiency that a combinatorial search operating blindly on content alone rarely matches exactly.

Restricting the comparison to the reduced 3D subspace $(\objtwo,\objthree,\objfour)$, however, reverses this outcome almost completely: the right column of Table~\ref{tab:input_dominance} shows dominance rates between $58.7\%$ and $99.5\%$. Once $\objone$ is set aside, \methodName's frontier therefore contains, in the large majority of runs, at least one solution that simultaneously improves on the input page in editorial style ($\objtwo$), text capacity ($\objthree$), and positioning ($\objfour$).

Beyond what this quantitative analysis alone can establish, informal discussions with \melody team suggest an explanation for this asymmetry: operators do pay close, conscious attention to the geometric occupation of the page, since it is the one property directly visible while editing, which is consistent with the near-perfect $\objone$ scores observed above. The permissiveness of the Melody authoring interface, however, does not strictly enforce the remaining objectives to the same extent, letting operators overflow text blocks beyond their nominal capacity ($\objthree$), introduce new or unusual templates rather than the historically recommended ones ($\objtwo$), or freely position the main article ($\objfour$). This interpretation is not derivable from the dataset itself and is reported here solely as context gathered from the industrial partner.

\subsection{Qualitative results}

This section provides a few examples of the pages synthesized by the framework. \figref{fig:qualitative_2art}, \figref{fig:qualitative_4art}, \figref{fig:qualitative_5art}, and \figref{fig:qualitative_8art} present four representative instances of increasing complexity ($\numarticles = 2, 4, 5, 8$). For each instance, the original input page is shown alongside three design alternatives (A, B, C) drawn from the non-dominated frontier produced by \methodName, together with their corresponding objective scores ($\objone, \objthree, \objfour, \objtwo$), visually illustrating the structural diversity and design plasticity achieved by \methodName.
...
While the quantitative and qualitative findings validate the efficiency of \methodNameGeneral, two primary aspects require a critical analysis regarding its deployment and extensibility.
